...
(function (window) {
  ...
  */
  ...
  var getNode_withAttr = function getNode_withAttr(lib_attr) {
    var all_nodes = document.getElementsByTagName('*');
    for (var i = 0, n = all_nodes.length; i < n; i++) {
  ...
  var source_code = getNode_withAttr("data-banach");
  ...
  if (banach_cfg.gui) {
    (function () {
      ...
      var toggleClicked = function toggleClicked(element) {
      ...
      var printAll_button = document.createElement('div');
      ...
      var printInvoked_button = document.createElement('div');
      ...
      var printInvokedNames_button = document.createElement('div');
      ...
      var performance_check_button = document.createElement('div');
      ...
      var invoking_path_button = document.createElement('div');
      ...
      invoking_path_button.onclick = function (e) {
        ...
        var prepare_console = function prepare_console() {
          setTimeout(function () {
            invoking_path();
          }, 5000);
        };
      ...
      var removeCustomAlert = function removeCustomAlert() {
  ...
  if (source_code.getAttribute("data-banach") === "functional_analysis") {
    ...
    (function () {
      var printAll = function printAll() {
        ...
        var all_message = "1. " + fn_expr_decl_inv_exe[0] + "</br>";
        for (var i = 1; i < fn_expr_decl_inv_exe.length; i++) {
      ...
      var printInvoked = function printInvoked() {
        console.info("Banach> Found invoked functions from your code:");
        console.dir(fn_inv_exe);
        var inv_message = "1. " + fn_inv_exe[0] + "</br>";
        for (var i = 1; i < fn_inv_exe.length; i++) {
          inv_message += "\n" + (i + 1) + ". " + fn_inv_exe[i] + "</br>";
        }
        return inv_message;
      };

      var printInvokedNames = function printInvokedNames() {
        console.info("Banach> Found names of invoked functions from your code:");
        console.dir(fn_inv_name);
        var inv_names_message = "1. " + fn_inv_name[0] + "</br>";
        for (var i = 1; i < fn_inv_name.length; i++) {
      ...
      var performance_check = function performance_check() {
        var performance_studies = function performance_studies() {
          performance_start = performance.now();
          return performance_start;
        };
        var performance_studies_end = function performance_studies_end() {
        ...
        var performance_script = document.createElement("SCRIPT");
      ...
      var invoking_path = function invoking_path() {
        var cage = "";
        var injection_script = document.createElement("SCRIPT");
        var injection_fn = function injection_fn() {
          for (var i = 0; i < fn_inv_exe.length; i++) {
            cage += "debugger; \n" + fn_inv_exe[i] + "\n ";
      ...
      var code_lines = banach_cfg.source.match(/[\r\n]/g);
      ...
      var empty_lines = banach_cfg.source.match(/^\s*$/gm);
      ...
      if (banach_cfg.language === "es5") {
        fn_expr_decl_inv = /var\s\w{1,35}\s=\sfunction\s?\(\)|var\s\w{1,35}\s=\sfunction\s?\((\w{1,35}?|\S{1,35}?,?){1,10}?\)|\w{1,35}\s=\sfunction\s?\(\)|\w{1,35}\s=\sfunction\s?\((\w{1,35}?|\S{1,35}?,?){1,10}?\)|function\s\w{1,35}\s?\(\)|function\s\w{1,35}\s?\((\w{1,35}?|\S{1,35}?,?){1,10}?\)|\w{1,35}\s?\(\);?|\w{1,35}\s?\((\w{1,35}?|\S{1,35}?,?){1,10}?\);?/g;
      ...
      var fn_expr_decl_inv_exe = banach_cfg.source.match(fn_expr_decl_inv);
      ;
      if (banach_cfg.printAll) {
        printAll();
      }

      fn_inv_exe = fn_expr_decl_inv_exe.filter(function (el) {
        return el.indexOf("function") === -1 ? true : false;
      });

      fn_inv_exe = fn_inv_exe.map(function (el) {
        var b1 = el.match(/\(/g).length;
        var b2 = el.match(/\)/g).length;
        if (b1 === b2) {
          return el;
        } else if (b1 > b2) {
          el += ");";
      ...
      var fn_inv_name = fn_inv_exe.map(function (el) {
        el.toString();
        var name_only = el.slice(0, el.indexOf("("));
        return name_only;
      });
